...
use bevy_mod_raycast::{
    ...
    RaycastSystem,DebugCursorMesh,DebugCursor
};
...
fn setup(
    ...
    let mut custom_material: StandardMaterial = Color::rgb(0.0,0.0,1.0).into();
    custom_material.unlit = true;
    commands.spawn_bundle(PbrBundle{
        mesh: meshes.add(Mesh::from(shape::Icosphere{radius: 0.05,subdivisions: 5})),
        material: materials.add(custom_material),
        ..Default::default()
    })
    .insert(DebugCursorMesh::<MyRaycastSet>::default())
    .insert(DebugCursor::<MyRaycastSet>::default());
    commands
