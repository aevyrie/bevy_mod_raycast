...
fn toggle_debug_cursor(
    ...
    if buttons.just_pressed(MouseButton::Left) {
        if cursor_ray_cast.enabled == Enabled::Enabled {
            println!("Ray casting disabled");
            cursor_ray_cast.enabled = Enabled::Disabled;
            keyboard_ray_cast.enabled = Enabled::Disabled;
        } else {
            println!("Ray casting enabled");
            cursor_ray_cast.enabled = Enabled::Enabled;
            keyboard_ray_cast.enabled = Enabled::Enabled;
        };
    }

    #[cfg(feature = "debug")]
    if buttons.just_pressed(MouseButton::Right) {
        if cursor_ray_cast.debug == Debug::Cursor {
            println!("Debug cursor disabled");
            cursor_ray_cast.debug = Debug::None;
            keyboard_ray_cast.debug = Debug::None;
        } else {
            println!("Debug cursor enabled");
            cursor_ray_cast.debug = Debug::Cursor;
            keyboard_ray_cast.debug = Debug::Cursor;
        };
    }
